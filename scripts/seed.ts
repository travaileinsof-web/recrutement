  ...
  for (const j of jobsData) {
    ...
    const job = await db.job.create({
      data: {
        ...
        skills: (process.env.DATABASE_URL?.startsWith('file:') ? JSON.stringify(j.skills) : j.skills) as any,
        category: j.category,
...
- Anglais professionnel.`,
      ...
      experienceLevel: 'CONFIRME',
      salaryText: '45–55 k€',
      requiredSkills: (process.env.DATABASE_URL?.startsWith('file:') ? JSON.stringify(['Product Management', 'SaaS', 'Roadmap', 'Analytics']) : ['Product Management', 'SaaS', 'Roadmap', 'Analytics']) as any,
