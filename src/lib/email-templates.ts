...
<body style="margin:0;padding:0;background:#faf7f2;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;color:#1a1a1a;">
  <table width="100%" cellpadding="0" cellspacing="0" style="padding:32px 16px;">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:8px;overflow:hidden;border:1px solid #e6dfd2;">
        <tr><td style="background:#0f4c3a;padding:24px 32px;">
          <span style="font-size:20px;font-weight:700;color:#f7f3ec;font-family:Georgia,serif;">TalentForge</span>
        </td></tr>
        <tr><td style="padding:32px;">
          ${content}
        </td></tr>
        <tr><td style="padding:24px 32px;background:#f1ece3;color:#5b5b54;font-size:12px;">
...
function ctaButton(href: string, label: string): string {
  return `<a href="${escapeHtml(href)}" style="display:inline-block;background:#e07a2c;color:#ffffff;padding:12px 24px;border-radius:6px;font-weight:600;text-decoration:none;">${escapeHtml(label)}</a>`
}
...
const templates: Record<string, (data: Record<string, unknown>) => TemplateResult> = {
  SUBMISSION_CONFIRMATION: (data) => {
    ...
    const html = layout(`
      <h1 style="margin:0 0 16px;font-size:24px;color:#0f4c3a;">Proposition d’offre reçue ✅</h1>
      ...
      <p style="margin:0 0 8px;font-size:14px;color:#5b5b54;">Référence de suivi :</p>
      <p style="margin:0 0 24px;font-family:monospace;background:#f1ece3;padding:8px 12px;border-radius:4px;display:inline-block;">${escapeHtml(reference)}</p>
    `)
  ...
  NEW_SUBMISSION: (data) => {
    ...
    const title = data.title
    const html = layout(`
      <h1 style="margin:0 0 16px;font-size:24px;color:#0f4c3a;">Nouvelle soumission à examiner</h1>
  ...
  APPLICATION_CONFIRMATION: (data) => {
    ...
    const html = layout(`
      <h1 style="margin:0 0 16px;font-size:24px;color:#0f4c3a;">Candidature reçue ✅</h1>
      <p style="margin:0 0 12px;">Bonjour,</p>
      ...
      <p style="margin:0 0 8px;font-size:14px;color:#5b5b54;">Référence de votre candidature :</p>
      <p style="margin:0 0 24px;font-family:monospace;background:#f1ece3;padding:8px 12px;border-radius:4px;display:inline-block;">${escapeHtml(reference)}</p>
  ...
  NEW_APPLICATION: (data) => {
    ...
    const jobTitle = data.jobTitle
    const html = layout(`
      <h1 style="margin:0 0 16px;font-size:24px;color:#0f4c3a;">Nouvelle candidature reçue</h1>
  ...
  APPLICATION_STATUS_UPDATE: (data) => {
    ...
    const html = layout(`
      <h1 style="margin:0 0 16px;font-size:24px;color:#0f4c3a;">Mise à jour de votre candidature</h1>
      <p style="margin:0 0 12px;">Bonjour,</p>
      ...
      ${publicMessage ? `<p style="margin:0 0 16px;padding:12px;background:#f1ece3;border-radius:4px;">${escapeHtml(publicMessage)}</p>` : ''}
    `)
  ...
  SUBMISSION_APPROVED: (data) => {
    ...
    const title = data.title
    const html = layout(`
      <h1 style="margin:0 0 16px;font-size:24px;color:#0f4c3a;">Proposition approuvée 🎉</h1>
      <p style="margin:0 0 12px;">Bonjour,</p>
      ...
      <p style="margin:0 0 8px;font-size:14px;color:#5b5b54;">Référence :</p>
  ...
  SUBMISSION_REJECTED: (data) => {
    ...
    const title = data.title
    const html = layout(`
      <h1 style="margin:0 0 16px;font-size:24px;color:#0f4c3a;">Proposition non retenue</h1>
      <p style="margin:0 0 12px;">Bonjour,</p>
  ...
  SUBMISSION_NEEDS_CORRECTION: (data) => {
    ...
    const html = layout(`
      <h1 style="margin:0 0 16px;font-size:24px;color:#0f4c3a;">Corrections demandées</h1>
      <p style="margin:0 0 12px;">Bonjour,</p>
      ...
      ${correctionMessage ? `<p style="margin:0 0 16px;padding:12px;background:#f1ece3;border-radius:4px;">${escapeHtml(correctionMessage)}</p>` : ''}
      <p style="margin:0 0 8px;font-size:14px;color:#5b5b54;">Référence :</p>
      <p style="margin:0;"><code>${escapeHtml(reference)}</code></p>
    `)
  ...
  APPLICATION_TRACKING_RESEND: (data) => {
    ...
    const trackingUrl = process.env.APP_URL
      ...
      : `http://localhost:3000${trackingUrlPath}`
    const html = layout(`
      <h1 style="margin:0 0 16px;font-size:24px;color:#0f4c3a;">Votre lien de suivi</h1>
      <p style="margin:0 0 12px;">Bonjour,</p>
      ...
      ${ctaButton(trackingUrl, 'Suivre ma candidature')}
      <p style="margin:16px 0 0;font-size:12px;color:#5b5b54;">Si vous n’avez pas demandé ce lien, ignorez cet e-mail.</p>
    `)
  ...
  DEFAULT: (data) => {
    const html = layout(`
      <h1 style="margin:0 0 16px;font-size:24px;color:#0f4c3a;">Notification TalentForge</h1>
      <pre style="white-space:pre-wrap;font-family:monospace;background:#f1ece3;padding:12px;border-radius:4px;">${escapeHtml(JSON.stringify(data, null, 2))}</pre>
    `)
