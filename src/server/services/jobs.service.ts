...
import { parseJsonArray } from '@/lib/normalize'
...
export async function createJobFromSubmission(submissionId: string, adminId: string) {
  ...
  const job = await db.job.create({
    data: {
      ...
      skills: submission.requiredSkills,
...
export async function adminSetJobStatus(jobId: string, status: string, adminId: string) {
  const before = await db.job.findUnique({ where: { id: jobId } })
  if (!before) throw new Error('JOB_NOT_FOUND')
  const patch: any = { status, updatedById: adminId }
...
export async function adminListJobs(filters: { search?: string; status?: string; page?: number; pageSize?: number }) {
  const page = Math.max(1, filters.page ?? 1)
